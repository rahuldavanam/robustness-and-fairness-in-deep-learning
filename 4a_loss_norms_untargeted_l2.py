...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            benign_loss_tensor = torch.norm(benign_loss_tensor,p=2)
            ...
            loss_tensor = criterion(adv_outputs, targets)
            for i in range(num_classes):
                adv_loss_tensor[i] = torch.sum(loss_tensor[targets==i])
            adv_loss_tensor = torch.norm(adv_loss_tensor, p=2)
...
if __name__ == '__main__':        
    ...
    print(f"Time taken for 60 epochs for untargeted squared loss = {end_time/60} minutes")