...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            for i in range(num_classes):
                benign_loss_tensor[i] = torch.sqrt(torch.sum(loss_tensor[targets==i]))
            ...
            loss_tensor = criterion(adv_outputs, targets)
            for i in range(num_classes):
                adv_loss_tensor[i] = torch.sqrt(torch.sum(loss_tensor[targets==i]))
...
if __name__ == '__main__':        
    ...
    print(f"Time taken for 60 epochs for untargeted square root loss = {end_time/60} minutes")